...
onBeforeMount(async () =>
{
  ...
  canResetPassword.value = await connectedUserStore.canResetPassword(props.userId, props.resetPasswordToken)
});
...
<template>
  <pollit-page
    :show-spinner-on-load="false"
    vertical-center
    :has-back-button="false"
    text-center
  >
    <label v-if="successFullyresetPassword">Your password was reset successfully.</label>
    ...
    <label v-else>Invalid reset password link. It may be that the link has expired.</label>
  </pollit-page>
</template>
